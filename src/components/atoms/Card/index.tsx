...
import { useUnlockNoteMutation } from "@redux/api/noteApi"; // ✅ IMPORT
...
export default function Card(props: any) {
  ...
  const [unlockNote] = useUnlockNoteMutation(); // ✅ API HOOK
  ...
  try {
    ...
    const unlockUntil =
      Date.now() + unlockValue.unlockMinutes * 60 * 1000;
  ...
  function handlePress() {
    if (props.isPasswordProtected && !isNotesUnlocked) {
      setShowLockedModal(true);
      return;
    }
  ...
  return (
    <>
      ...
      >
        ...
        <View style={styles.iconsWrap}>
          {props.isPasswordProtected && !isNotesUnlocked && (
            <Feather name="lock" size={24} />
          )}
          {props.isReminderSet && <Feather name="clock" size={24} />}
        </View>
      </TouchableOpacity>
    </>
  );
